...
namespace CoreBVN
{
    public class ActiveDirectoryQuery : IDisposable
    {
        ...
        private SearchResult SearchUserByStaffNumber(DirectorySearcher ds)
        {
            ...
            try
            {
                ds.Filter = "(&((&(objectCategory=Person)(objectClass=User)))( description=" + staffADProfile.employee_number + "))";
                ...
                if (userObject != null)
                {
                    logWriter.WriteErrorLog(string.Format("SearchUserByStaffNumber : One Found {0} ", staffADProfile.employee_number));
                }
                else
                {
                    logWriter.WriteErrorLog(string.Format("SearchUserByStaffNumber : None Found {0} ", staffADProfile.employee_number));
                }
            }
            catch (Exception ex)
            {
                logWriter.WriteErrorLog(string.Format("SearchUserByStaffNumber : Exception!!! {0} / {1}", ex.Message, staffADProfile.employee_number));
            }
        ...
        public DirectorySearcher GetDirectorySearcher(string username, string password, string domain)
        {
            ...
            if (dirSearch == null)
            {
                ...
                {
                    logWriter.WriteErrorLog(string.Format("Connection Creditial is Wrong!!!, please Check."));
                }
                return dirSearch;
